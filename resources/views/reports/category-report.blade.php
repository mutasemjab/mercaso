        ...
        <div class="row mb-4 no-print">
            <div class="col-12">
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body">
                        <form method="GET" action="{{ route('category-report.index') }}"
                            class="form-inline justify-content-center flex-wrap">
                            <div class="form-group {{ app()->getLocale() == 'ar' ? 'ml-3' : 'mr-3' }} mb-2">
                                <label for="start_date"
                                    class="{{ app()->getLocale() == 'ar' ? 'ml-2' : 'mr-2' }} font-weight-bold">
                            ...
                            <div class="form-group {{ app()->getLocale() == 'ar' ? 'ml-3' : 'mr-3' }} mb-2">
                                ...
                                <select name="user_type" id="user_type" class="form-control">
                                    <option value="">{{ __('messages.all') }}</option>
                                    <option value="1" @selected($userType == 1)>{{ __('messages.retail') }}
                                    </option>
                                    <option value="2" @selected($userType == 2)>{{ __('messages.wholesale') }}
                                    </option>
                                </select>
                            </div>

                            <button type="submit"
                                class="btn btn-primary btn-lg {{ app()->getLocale() == 'ar' ? 'ml-2' : 'ml-2' }} mb-2">
                                <i class="fas fa-sync-alt mr-2"></i>{{ __('messages.Generate_Report') }}
                            </button>

                            <a href="{{ route('category-report.index') }}"
                                class="btn btn-outline-secondary btn-lg {{ app()->getLocale() == 'ar' ? 'ml-2' : 'ml-2' }} mb-2">
                ...
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped mb-0 table-hover report-table"
                                id="reportTable">
                                ...
                                <tbody>
                                    @forelse($categoryReport as $index => $category)
                                        @include('reports.partials.category-row', [
                                            'category' => $category,
                                            'index' => $index + 1,
                                        ])
                                    @empty
                                    ...
                                    @if (!empty($categoryReport))
                                        <tr class="total-row">
                                            ...
                                            <td class="text-right font-weight-bold">
                                                {{ number_format($totals['total_quantity_sold']) }}</td>
                                            <td class="text-right font-weight-bold">
                                                ${{ number_format($totals['total_sales_before_tax'], 2) }}</td>
                                            <td class="text-right font-weight-bold">
                                                ${{ number_format($totals['total_discount'], 2) }}</td>
                                            <td class="text-right font-weight-bold">
                                                ${{ number_format($totals['total_tax'], 2) }}</td>
                                            <td class="text-right font-weight-bold">
                                                ${{ number_format($totals['total_sales_after_tax'], 2) }}</td>
                                            <td class="text-center font-weight-bold no-print">
                                                {{ number_format($totals['total_orders']) }}</td>
                                            <td class="text-center font-weight-bold no-print">
                                                {{ number_format($totals['total_products']) }}</td>
                                            <td class="text-center font-weight-bold no-print">
                                                {{ number_format($totals['total_in_stock']) }}</td>
                                            <td class="text-center font-weight-bold no-print">
                                                {{ number_format($totals['total_out_of_stock']) }}</td>
                                        </tr>
                                    @endif
                                </tbody>
                            </table>
        ...
        <div class="row mt-4 no-print">
            <div class="col-12 text-center">
                <form method="GET" action="{{ route('category-report.export') }}" style="display: inline;">
                    ...
                    <button type="submit"
                        class="btn btn-success btn-lg {{ app()->getLocale() == 'ar' ? 'ml-2' : 'mr-2' }}">
