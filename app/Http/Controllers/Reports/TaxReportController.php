        ...
        if ($shopId) {
            ...
            foreach ($taxes as $tax) {
                ...
                $refundOrders = OrderProduct::whereIn('order_id', $orders->pluck('id'))
                  ...
                  // Check if tax is zero before dividing
                $salesTotal = $salesOrders->sum(function ($orderProduct) use ($tax) {
                    return $tax != 0 ? $orderProduct->tax_value / ($tax / 100) : 0;
                });

              //  $salesTotal = $salesOrders->sum('total_price_before_tax');
                $refundTotal = $refundOrders->sum('total_price_before_tax');
